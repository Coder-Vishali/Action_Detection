...
def main():
    ...
    model_path = '.optical_checkpoints/model_best.pth.tar'
    data_dir = "./../../datasets/gms_frames"
    start_frame = 0
    num_categories = 3

    model_start_time = time.time()
    params = torch.load(model_path)
    temporal_net = models.flow_resnet152(pretrained=False, num_classes=3)
    ...
    print("Total match count: ", match_count)
    print("Total vidoes count: ", len(val_list))
    print("Accuracy is %4.4f" % (float(match_count)/len(val_list)))
    np.save("gms_s1_flow_resnet152.npy", np.array(result_list))
