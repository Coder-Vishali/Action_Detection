...
def VideoSpatialPrediction(
    ...
    if num_frames == 0:
        ...
        imagelist = []
        for each in imglist:
            if each.split("_",1)[0] == "img":
                imagelist.append(each)
        duration = len(imagelist)
        # print("imglist: ",imglist)
        # duration = len(imglist)
        # print("if condition - Duration : ",duration)
    else:
        duration = num_frames
        # print("else condition - Duration: ", duration)
    ...
    for i in range(num_samples):
        # print("step: ", step)
        # print("i * step: ", i * step + 1)
        img_file = os.path.join(vid_name, 'img_{0:d}.jpg'.format(i*step+1))
